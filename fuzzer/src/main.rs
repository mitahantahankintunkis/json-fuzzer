...
fn fuzz(stream: &mut TcpStream, args: &Args, payload_config: &PayloadConfig) -> Encoder {
    ...
    let payload_size = payload.into_iter().collect::<Vec<u8>>().len();
        ...
        if has_next && (sent_messages - received_messages) < in_buffer_count {
            ...
            for _ in 0..(in_buffer_count - (sent_messages - received_messages)) {
                for _ in 0..batch_size {
                    ...
                    // send_buffer[sent_bytes..(sent_bytes + payload.fuzzed_length())]
                    //     .copy_from_slice(payload.into_iter());
...
fn handle_client(stream: &mut TcpStream, args: &Args) {
    ...
    for payload_config in &config.payloads {
        ...
        let compressed = fuzz(stream, &args, &payload_config);
